...
# with torch.no_grad():  # disable gradient back-propagation
    ...
    for batch, (x, label) in enumerate(target_bar):
        ...
        shared_f_t, private_f_t, = model(x, 2)
        ...
        f = mask_channels(shared_f_t, scores_t, filter_num)
        y = classifier(f)[-1]
        print("Num of FLOPs of {} ({}) is {:.2f}M.".
              format(CFG.MODEL.NAME, CFG.MODEL.BACKBONE,
                     count_flops(model, x, 2) + count_flops(domain_classifier, shared_f_t) +
                     count_flops(classifier, f)))
        raise NotImplementedError
        pred = y.argmax(axis=1)
        f = torch.squeeze(f)
        data_t.append(x.data.cpu().numpy())
    ...
    for i in range(NUM_CLASSES):
        ...
        xs = data_s[np.where(labels_s == i)]
        xt = data_t[np.where(labels_t == i)]
        # np.random.shuffle(fs)
        # np.random.shuffle(ft)
        xs, xt = xs[:args.sample_number], xt[:args.sample_number]
        x = np.concatenate([xs, xt], axis=0)
        x = x.reshape(x.shape[0], -1)
        print(f.shape, x.shape)
        tsne = TSNE(n_components=2)
        x = tsne.fit_transform(x)
        xs, xt = x[:num_fs], x[num_fs:]
        plot_features(xs, xt, os.path.join(args.path, 'data_class{}.png'.format(i)))
