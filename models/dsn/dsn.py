...
class DSN(nn.Module):
    def __init__(self, num_classes: int, experts: List[nn.Module]):
        ...
        self.shared_decoder = nn.Sequential(
            nn.ConvTranspose2d(self.out_channels, 256, 3, 2, 0),  # 1*1 ==> 2*2
            nn.BatchNorm2d(256),
            self.relu,

            nn.ConvTranspose2d(256, 128, 3, 2, 1),  # 2*2 ==> 4*4
            nn.BatchNorm2d(128),
            self.relu,

            nn.ConvTranspose2d(128, 64, 3, 2, 1),  # 4*4 ==> 8*8
            nn.BatchNorm2d(64),
            self.relu,

            nn.ConvTranspose2d(64, self.num_channels, 3, 2, 1),  # 8*8 ==> 16*16
        )
    ...
    def forward(self, x, task_ind):
        ...
        decoder_output = self.shared_decoder(shared_features + private_features)
        class_output = self.classifier(shared_features)
        reverse_shared_features = self.grl(shared_features)
        domain_output = self.domain_discriminator(reverse_shared_features)
