...
def worker(rank_gpu, args):
    ...
    while True:
        ...
        for iteration in train_bar:
            ...
            scores_t = cal_grad_scores(shared_f_t_, domain_out_t, 2, False)
            ...
            sim_loss = similarity_criterion(f_s=masked_shared_f_s, f_t=masked_shared_f_t, label_s=label_s,
                                            y_s=y_s, y_t=y_t) * loss_weights[1]
            difference_s_loss = difference_criterion(masked_shared_f_s, masked_private_f_s) * loss_weights[2]
            difference_t_loss = difference_criterion(masked_shared_f_t, masked_private_f_t) * loss_weights[2]
            step1_loss = cls_s_loss + sim_loss + difference_s_loss + difference_t_loss

            step1_loss.backward()
            optimizer_model.step()
            cls_s_loss_epoch += cls_s_loss.item()
            sim_loss_epoch += sim_loss.item()
