...
def worker(rank_gpu, args):
    ...
    while True:
        ...
        for iteration in train_bar:
            ...
            optimizer.zero_grad()
            shared_f_s, private_f_s, y_s, domain_out_s, scores_s = model(x_s, 1, domain_label_s)
            shared_f_t, private_f_t, y_t, domain_out_t, scores_t = model(x_t, 2, domain_label_t)
            print(scores_s, scores_t)
