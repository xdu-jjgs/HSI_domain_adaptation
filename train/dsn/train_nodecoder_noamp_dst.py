...
def worker(rank_gpu, args):
    ...
    difference_criterion = build_criterion(loss_names[3])
    ...
    while True:
        ...
        (total_loss_epoch, cls_s_loss_epoch, worst_loss_epoch, cbst_loss_epoch,
         difference_s_loss_epoch, difference_t_loss_epoch) = 0., 0., 0., 0., 0., 0.
        for iteration in train_bar:
            ...
            optimizer.zero_grad()
            shared_f_s, private_f_s, y_s, _, y_s_adv = model(x_s, 1)
            ...
            difference_t_loss = difference_criterion(shared_f_t, private_f_t) * loss_weights[3]
            total_loss = cls_s_loss + cbst_loss + worst_loss + difference_s_loss + difference_t_loss
        ...
        if dist.get_rank() == 0:
            ...
            writer.add_scalar('train/loss_wcec-epoch', worst_loss_epoch, epoch)
        ...
        logging.info(
            'rank{} train epoch={} | loss_total={:.3f} loss_cls_s={:.3f} loss_cbst={:.3f} loss_wos={:.3f} '
            'loss_difference_s={:.3f} loss_difference_t={:.3f}'
            .format(dist.get_rank() + 1, epoch, total_loss_epoch, cls_s_loss_epoch, cbst_loss_epoch,
                    worst_loss_epoch, difference_s_loss_epoch, difference_t_loss_epoch))
        logging.info(
