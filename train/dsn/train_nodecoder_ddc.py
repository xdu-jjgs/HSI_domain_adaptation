...
def worker(rank_gpu, args):
    ...
    while True:
        ...
        total_loss_epoch, cls_s_loss_epoch, sim_loss_epoch, difference_s_loss_epoch, difference_t_loss_epoch = \
            0., 0., 0., 0., 0.
        for iteration in train_bar:
            ...
            with autocast():
                ...
                total_loss = cls_s_loss + sim_loss + difference_s_loss + difference_t_loss
        ...
        logging.info(
            'rank{} train epoch={} | loss_total={:.3f} loss_cls_s={:.3f} loss_sim={:.3f} loss_difference_s={:.3f} '
            'loss_difference_t={:.3f}'.format(dist.get_rank() + 1, epoch, total_loss_epoch, cls_s_loss_epoch,
                                              sim_loss_epoch, difference_s_loss_epoch, difference_t_loss_epoch))
        logging.info(
        ...
        with torch.no_grad():  # disable gradient back-propagation
            for x_t, label_t in val_bar:
                ...
                with autocast():
                    _, _, y_t = model(x_t, 2)
                    cls_s_loss = val_criterion(y_s=y_t, label_s=label_t)
