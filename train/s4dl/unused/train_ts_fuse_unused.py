...
from models.utils.functions import adjusted_sigmoid, update_ema
...
def worker(rank_gpu, args):
    ...
    difference_criterion = build_criterion(loss_names[1])
    difference_criterion.to(device)
    domain_criterion = build_criterion(loss_names[2])
    domain_criterion.to(device)
    mmd_criterion = build_criterion(loss_names[3])
    mmd_criterion.to(device)
    cbst_criterion = build_criterion(loss_names[4])
    ...
    while True:
        ...
        (step1_loss_epoch, cls_loss_epoch, difference_s_loss_epoch, difference_t_loss_epoch,
         domain_s_loss_epoch, domain_t_loss_epoch, mmd_loss_epoch, cbst_loss_epoch) = 0., 0., 0., 0., 0., 0., 0., 0.
        ...
        for iteration in train_bar:
            ...
            features_di_s, features_ds_s, = model(x_s, 1)
            features_di_t, _, = model(x_t, 2)
            domain_out_s = domain_classifier(features_di_s)[-1]
            domain_out_t = domain_classifier(features_di_t)[-1]

            features_s.append(features_di_s.data.cpu().numpy())
            features_t.append(features_di_t.data.cpu().numpy())
            scores_s = cal_grad_scores(features_di_s, domain_out_s, 1)
            scores_t = cal_grad_scores(features_d_t, domain_out_t, 2)
            filter_num = int(model.module.filter_ratio * scores_s.size()[1])
            masked_shared_f_s = mask_channels(features_di_s, scores_s, filter_num)
            masked_shared_f_t = mask_channels(features_di_t, scores_t, filter_num)
            scores_s_abs = scores_s.abs()
            scores_t_abs = scores_t.abs()
            masked_private_f_s = mask_channels(features_ds_s, scores_s_abs, filter_num, largest=False)
            masked_private_f_t = mask_channels(features_di_t, scores_t_abs, filter_num, largest=False)

            y_s = classifier(masked_shared_f_s)[-1]
            y_t = classifier(masked_shared_f_t)[-1]

            cls_loss = cls_criterion(y_s=y_s, label_s=label_s) * loss_weights[0]
            difference_s_loss = difference_criterion(masked_shared_f_s, masked_private_f_s) * loss_weights[1]
            difference_t_loss = difference_criterion(masked_shared_f_t, masked_private_f_t) * loss_weights[1]
            mmd_loss = (mmd_criterion(f_s=masked_shared_f_s, f_t=masked_shared_f_t, label_s=label_s, y_s=y_s, y_t=y_t)
                        * loss_weights[3])
            cbst_loss, _, _ = cbst_criterion(y_t, y_t)
            cbst_loss *= loss_weights[4]
            step1_loss = (cls_loss + difference_s_loss + difference_t_loss + mmd_loss + cbst_loss)
            ...
            optimizer_domain_classifier.zero_grad()
            domain_out_s = domain_classifier(features_di_s.detach())[-1]
            domain_out_t = domain_classifier(features_di_t.detach())[-1]
            domain_s_loss = domain_criterion(y_s=domain_out_s, label_s=domain_label_s) * loss_weights[2]
            domain_t_loss = domain_criterion(y_s=domain_out_t, label_s=domain_label_t) * loss_weights[2]
        ...
        mmd_loss_epoch /= iteration * CFG.DATALOADER.BATCH_SIZE
        ...
        features_t = np.concatenate(features_t)
        features_mix = np.concatenate([features_s, features_t])
        std_mix = np.std(features_mix, axis=0)
        mean_std_mix = np.mean(std_mix)
        assert CFG.HYPERPARAMS[1] >= 0.
        assert CFG.HYPERPARAMS[2] >= 0.
        assert 0. <= CFG.HYPERPARAMS[3] < 1.
        cur_ratio = adjusted_sigmoid(mean_std_mix, k=CFG.HYPERPARAMS[1], s=CFG.HYPERPARAMS[2])
        filter_ratio = update_ema(model.module.filter_ratio, cur_ratio, CFG.HYPERPARAMS[3])
        model.module.filter_ratio = filter_ratio
        logging.info("rank{} train epoch={} | std:{:.3f} cur_ratio:{} filter_ratio:{}"
                     .format(dist.get_rank() + 1, epoch, mean_std_mix, cur_ratio, model.module.filter_ratio))
        ...
        if dist.get_rank() == 0:
            ...
            writer.add_scalar('train/loss_mmd-epoch', mmd_loss_epoch, epoch)
            ...
            writer.add_scalar('train/mean-std-epoch', mean_std_mix, epoch)
            writer.add_scalar('train/cur_ratio-epoch', cur_ratio, epoch)
            writer.add_scalar('train/filter_ratio-epoch', model.module.filter_ratio, epoch)
        logging.info(
            'rank{} train epoch={} | '
            'loss_s1={:.3f} loss_cls={:.3f} loss_difference_s={:.3f} loss_difference_t={:.3f} '
            'loss_domain_s={:.3f} loss_domain_t={:.3f} loss_mmd={:.3f} loss_cbst={:.3f}'.format(
                dist.get_rank() + 1, epoch, step1_loss_epoch, cls_loss_epoch,
                difference_s_loss_epoch, difference_t_loss_epoch, domain_s_loss_epoch, domain_t_loss_epoch,
                mmd_loss_epoch, cbst_loss_epoch
            )
        )
        logging.info(
        ...
        for x_t, label_t in val_bar:
            ...
            masked_shared_f_t = mask_channels(shared_f_t, scores_t, filter_num)
            y_t = classifier(masked_shared_f_t)[-1]
