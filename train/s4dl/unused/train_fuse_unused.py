...
def worker(rank_gpu, args):
    ...
    while True:
        ...
        (total_loss_epoch, cls_loss_epoch, difference_s_loss_epoch, difference_t_loss_epoch,
        ...
        for iteration in train_bar:
            ...
            optimizer.zero_grad()
            masked_shared_f_s, masked_private_f_s, y_s, domain_out_s, shared_f_s, private_f_s = \
                model(x_s, 1, True, True, True)
            masked_shared_f_t, masked_private_f_t, y_t, domain_out_t, shared_f_t, private_f_t = \
                model(x_t, 2, True, True, True)
            ...
            mmd_loss = (mmd_criterion(f_s=masked_shared_f_s, f_t=masked_shared_f_t, label_s=label, y_s=y_s, y_t=y_t)
                        * loss_weights[3])
            cbst_loss, _, _ = cbst_criterion(y_t, y_t)
            cbst_loss *= loss_weights[4]

            total_loss = (cls_loss + difference_s_loss + difference_t_loss +
                          domain_s_loss + domain_t_loss + mmd_loss + cbst_loss)
            ...
            features_s.append(shared_f_s.data.cpu().numpy())
            features_t.append(shared_f_t.data.cpu().numpy())
        ...
        logging.info(
            'rank{} train epoch={} | '
            'loss_total={:.3f} loss_cls={:.3f} loss_difference_s={:.3f} loss_difference_t={:.3f} '
