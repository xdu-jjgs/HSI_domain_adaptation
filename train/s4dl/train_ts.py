...
def worker(rank_gpu, args):
    ...
    while True:
        ...
        step1_loss_epoch, cls_s_loss_epoch, sim_loss_epoch, domain_s_loss_epoch, \
        ...
        for iteration in train_bar:
            ...
            scores_t = cal_grad_scores(shared_f_t_, domain_out_t, 2)
            filter_num = int(model.module.filter_ratio * scores_s.size()[1])
            masked_shared_f_s = mask_channels(shared_f_s, scores_s, filter_num)
            masked_shared_f_t = mask_channels(shared_f_t, scores_t, filter_num)
            scores_s_abs = scores_s.abs()
            scores_t_abs = scores_t.abs()
            masked_private_f_s = mask_channels(private_f_s, scores_s_abs, filter_num, largest=False)
            masked_private_f_t = mask_channels(private_f_t, scores_t_abs, filter_num, largest=False)
            ...
            step1_loss = cls_s_loss + difference_s_loss + difference_t_loss
        ...
        step1_loss_epoch /= iteration
        cls_s_loss_epoch /= iteration
        sim_loss_epoch /= iteration
        domain_s_loss_epoch /= iteration
        domain_t_loss_epoch /= iteration
        difference_s_loss_epoch /= iteration
        difference_t_loss_epoch /= iteration
        ...
        if dist.get_rank() == 0:
            ...
            writer.add_scalar('train/loss_sim-epoch', sim_loss_epoch, epoch)
        ...
        logging.info(
            'rank{} train epoch={} | loss_s1={:.3f} loss_cls_s={:.3f} loss_sim={:.3f} '
            'loss_domain_s={:.3f} loss_domain_t={:.3f} loss_difference_s={:.3f} loss_difference_t={:.3f}'
            .format(dist.get_rank() + 1, epoch, step1_loss_epoch, cls_s_loss_epoch, sim_loss_epoch,
        ...
        confidence_sum /= len(val_dataloader)
