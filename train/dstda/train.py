...
def worker(rank_gpu, args):
    ...
    while True:
        ...
        total_loss_epoch, step1_loss_epoch, step2_loss_epoch, cls_loss_epoch, \
            worst_loss_epoch, domain_loss_epoch, cbst_loss_epoch = 0., 0., 0., 0., 0., 0., 0.
        for iteration in train_bar:
            ...
            f_s, y_s, _, y_s_adv_k, y_s_adv_d = model(x_s)
            f_t, y_t, y_pse, y_t_adv_k, y_t_adv_d = model(x_t)

            domain_label_s = torch.zeros(len(label_s))
            # len of label_s equal to label_t
            ...
            worst_loss = wcec_criterion(y_s, y_s_adv_k, y_t, y_t_adv_k)
            domain_s_loss = cls_criterion(y_s=y_s_adv_d, label_s=domain_label_s)
            domain_t_loss = cls_criterion(y_s=y_t_adv_d, label_s=domain_label_t)
            step1_loss = cls_loss + worst_loss + domain_s_loss + domain_t_loss
            ...
            pred_adv_s = y_s_adv_k.argmax(axis=1)
            pred_adv_t = y_t_adv_k.argmax(axis=1)
