...
set CONFIGS_FOLDER=configs/shanghang/dsn/hypers_tmp
set RUNS_FOLDER=runs/shanghang_hyper/
...
for %%f in (%CONFIGS_FOLDER%\*.yaml) do (
    ...
    python train\dsn\train_nodecoder_noamp_st_score2_open_ada_ema.py %%f ^
        --path !OUTPUT_PATH! ^
        --nodes 1 ^
        --gpus 1 ^
        --rank-node 0 ^
