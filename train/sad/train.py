...
def worker(rank_gpu, args):
    ...
    while True:
        ...
        total_loss_epoch, cls_loss_epoch, domain_s_loss_epoch, domain_t_loss_epoch, sar_s_loss_epoch, \
            sar_t_loss_epoch = 0., 0., 0., 0., 0., 0.
        for iteration in train_bar:
            ...
            optimizer.zero_grad()
            with autocast():
                di_s, y_s, domain_out_s, sar_di_s, sar_ds_s = model(x_s)
                di_t, y_t, domain_out_t, sar_di_t, sar_ds_t = model(x_t)
                ...
                sar_s_loss = (cls_criterion(y_s=sar_di_s, label_s=domain_label_s) +
                              cls_criterion(y_s=sar_ds_s, label_s=domain_label_s)) * loss_weights[2]
                sar_t_loss = (cls_criterion(y_s=sar_di_t, label_s=domain_label_t) +
                              cls_criterion(y_s=sar_ds_t, label_s=domain_label_t)) * loss_weights[2]
                total_loss = cls_loss + domain_s_loss + domain_t_loss + sar_s_loss + sar_t_loss

            cls_loss_epoch += cls_loss.item()
            domain_s_loss_epoch += domain_s_loss.item()
            domain_t_loss_epoch += domain_t_loss.item()
            sar_s_loss_epoch += sar_s_loss.item()
            sar_t_loss_epoch += sar_t_loss.item()
        ...
        sar_s_loss_epoch /= iteration * CFG.DATALOADER.BATCH_SIZE
        sar_t_loss_epoch /= iteration * CFG.DATALOADER.BATCH_SIZE
        ...
        if dist.get_rank() == 0:
            ...
            writer.add_scalar('train/loss_sar_s-epoch', sar_s_loss_epoch, epoch)
            writer.add_scalar('train/loss_sar_t-epoch', sar_t_loss_epoch, epoch)
