...
def worker(rank_gpu, args):
    ...
    contrast_criterion = build_criterion(loss_names[1])
    contrast_criterion.to(device)
    ...
    while True:
        ...
        total_loss_epoch, cls_loss_epoch, contrast_loss_epoch = 0., 0., 0.
        for iteration in train_bar:
            ...
            optimizer.zero_grad()
            with autocast():
                di_s, ds_s, y_s = model(x_s)
                # di_t, ds_t, y_t = model(x_t)
                domain_label_di = torch.zeros(len(label))
                domain_label_ds = torch.ones(len(label))
                domain_label = torch.concatenate([domain_label_di, domain_label_ds], dim=0)
                domain_label = domain_label.to(device)

                cls_loss = cls_criterion(y_s=y_s, label_s=label) * loss_weights[0]
                contrast_loss = contrast_criterion(torch.concatenate([di_s, ds_s]), domain_label) * loss_weights[1]
                total_loss = cls_loss + contrast_loss

            cls_loss_epoch += cls_loss.item()
            contrast_loss_epoch += contrast_loss.item()
        ...
        contrast_loss_epoch /= iteration * CFG.DATALOADER.BATCH_SIZE
        ...
        if dist.get_rank() == 0:
            ...
            writer.add_scalar('train/loss_contrast-epoch', contrast_loss_epoch, epoch)
        ...
        logging.info(
            'rank{} train epoch={} | loss_total={:.3f} loss_cls={:.3f} loss_contrast={:.3f} '.format(
                dist.get_rank() + 1, epoch, total_loss_epoch, cls_loss_epoch, contrast_loss_epoch
            ))
        logging.info(
