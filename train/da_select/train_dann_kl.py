...
def worker(rank_gpu, args):
    ...
    kldiv_criterion = build_criterion('kldiv')
    kldiv_criterion.to(device)
    ...
    while True:
        ...
        for iteration in train_bar:
            ...
            x_select = x_t[select_mask == 1]
            y_select = y_t.argmax(axis=1)[select_mask == 1]
            label_select = label_t[select_mask == 1]
            for d, l in zip(x_select, y_select):
                select_dataset.append(d, l)
            metric_selector.add(y_select.data.cpu().numpy(), label_select.data.cpu().numpy())

            y_confid, _ = torch.max(y_t_, dim=1)
            y_doubt = 1.0 - y_confid
            y_confid = torch.stack((y_doubt, y_confid), dim=1)
            # TODO: or l2dis
            confid_loss = kldiv_criterion(select_status, y_confid)
            step2_loss = confid_loss
        ...
        logging.info(
            'rank{} train selector epoch={} | PA={:.3f} mPA={:.3f} KC={:.3f}'.format(dist.get_rank() + 1, epoch, PA,
                                                                                     mPA, KC))
        for c in range(NUM_CLASSES):
            logging.info(
                'rank{} train selector epoch={} | class={} P={:.3f} R={:.3f} F1={:.3f}'.format(dist.get_rank() + 1,
                                                                                               epoch, c,
                                                                                               Ps[c], Rs[c], F1S[c]))
