...
def worker(rank_gpu, args):
    ...
    # mixed precision
    [dann, selector], [optimizer_model, optimizer_selector] = amp.initialize(
        [dann, selector],
        [optimizer_model,
         optimizer_selector],
    ...
    while True:
        ...
        iteration = 0
        # TODO: try to use soft label? try train dann?
        for x_t, pseudo_labels_select in selected_bar:
            iteration += 1
            p = float(iteration + epoch * len(select_dataset)) / CFG.EPOCHS / len(select_dataset)
            alpha = 2. / (1. + np.exp(-10 * p)) - 1
            f_t, y_select, _ = dann(x_t, alpha=alpha)

            step3_loss = cls_criterion(y_s=y_select, label_s=pseudo_labels_select)
            optimizer_model.zero_grad()
            with amp.scale_loss(step3_loss, optimizer_model) as scaled_loss:
                scaled_loss.backward()
            optimizer_model.step()
